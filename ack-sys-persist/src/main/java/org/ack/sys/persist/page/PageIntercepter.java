...
import org.ack.sys.util.ReflectUtil;
 ...
 * <p>
 * 利用拦截器实现Mybatis分页的原理： 要利用JDBC对数据库进行操作就必须要有一个对应的Statement对象，
...
public class PageIntercepter implements Interceptor {
	...
	public String dataBaseType;// 数据库类型 oralce or mysql
	...
	public Object intercept(Invocation invocation) throws Throwable {
		 ...
		 * boundSql，这里不管是调用handler.getBoundSql()还是直接调用delegate
		 * .getBoundSql()结果是一样的，因为之前已经说过了
	...
	public String getPageSql(Page<?> page, String sql) {
		...
		if ("mysql".equalsIgnoreCase(dataBaseType)) {
			...
			return getPostgresqlPageSql(page, sb);
		}
		return sb.toString();
	}

	public String getOraclePageSql(Page<?> page, StringBuilder sb) {
		...
		// 上面添加后结果 select t.*, rownum r from (select * from t_user) t where
		// rownum < 31
		sb.insert(0, " select * from ( ");
		sb.append(" ) where r >= "); // 开始查询位置
		sb.append(offSet);
		// 上面添加结果 select * from (select t.*, rownum r from (select * from
		// t_user) t where rownum < 31) where r >= 16
	 ...
	 * @return
	 */
	public String getMysqlPageSql(Page<?> page, StringBuilder sb) {
		// 计算第一条记录的位置，Mysql中记录的位置是从0开始的。
		int offSet = (page.getCurrentPage() - 1) * page.getPageSize();
		
		String orderColumn = page.getOrderColumn();
		String orderType = (page.getOrderType()==null ? "asc" : page.getOrderType());
		if(null != orderColumn && !"".equals(orderColumn)){
			sb.append(" order by " + orderColumn  + " " + orderType);
		}
	 ...
	 * @return
	 */
	public String getPostgresqlPageSql(Page<?> page, StringBuilder sb) {
	 ...
	 * @param connection
	 */
	public void setTotalRecord(Page<?> page, MappedStatement mappedStatement,
		...
		try {
			...
			if (rs.next()) {
				...
				// 设置总页数
				int p = totalRecord % page.getPageSize();
				int totalPage = 1;
				if (p == 0) {
	...
	public void setFieldValue(BoundSql obj, String fieldName, String pageSql) {
