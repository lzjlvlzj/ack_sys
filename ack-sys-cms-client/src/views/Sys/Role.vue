<template>
  <div class="page-container">
	...
	<!--表格内容栏-->
	<kt-table :height="220" permsEdit="sys:role:edit" permsDelete="sys:role:delete"
            :btns="btns" :btnWidth="btnWidth"
            :highlightCurrentRow="true" :stripe="true"
...
export default {
	...
	data() {
        ...
        {prop:"weight", label:"角色权重", minWidth:120},
				{prop:"creator", label:"创建人", minWidth:120},
				{prop:"createTime", label:"创建时间", minWidth:120, formatter:this.dateFormat}
				// {prop:"lastUpdateBy", label:"更新人", minWidth:100},
				// {prop:"lastUpdateTime", label:"更新时间", minWidth:120, formatter:this.dateFormat}
			],
			pageRequest: { currentPage: 1, pageSize: 5 },
			pageResult: {},
      btnWidth : 400,
      btns:[
        {
        ...
        weight:[
          { required: true, message: '请输入角色权重值', trigger: 'blur' }
        ]
			},
			// 新增编辑界面数据
			dataForm: {
	...
	methods: {
      ...
      this.$api.role.saveRoleMenus(role).then((res) => {
        if(res.code == 200) {
          this.$message({ message: '操作成功', type: 'success' })
        } else {
          this.$message({message: '操作失败, ' + res.msg, type: 'error'})
